...
class PCMAudioCapture {
   ...
   *
   * The audio pipeline has latency: Mic → MediaStream → ScriptProcessorNode.
   * When the user releases the hotkey, audio may still be "in flight" between
   * the microphone and the processor's buffer. We handle this in two phases:
   *
   * Phase 1: Drain delay - wait for in-flight audio to reach the processor
   * Phase 2: Silence injection - push any partial buffer through
   *
  ...
  async stopAndFlush(): Promise<void> {
    ...
    // Phase 1: Drain delay
    // Audio travels through: Mic → MediaStream → ScriptProcessorNode buffer.
    // When we disconnect the source, any audio not yet in the processor's
    // buffer is lost. Wait for 2 buffer cycles to allow in-flight audio
    // to be captured before disconnecting.
    //
    // Buffer cycle duration = BUFFER_SIZE_SAMPLES / sampleRate
    // At 2048 samples / 16kHz = 128ms per cycle
    // Waiting 2 cycles = 256ms ensures audio in the pipeline is captured.
    const DRAIN_DELAY_MS =
      (AUDIO_BUFFER_CONFIG.BUFFER_SIZE_SAMPLES / 16000) * 1000 * 2;
    void debugLogger.log("PCM_DRAIN_START", { drainDelayMs: DRAIN_DELAY_MS });
    await new Promise((r) => setTimeout(r, DRAIN_DELAY_MS));
    void debugLogger.log("PCM_DRAIN_COMPLETE");

    // Signal that we're stopping - onaudioprocess will process remaining chunks
    this.isStopping = true;

    // Phase 2: Silence injection
    // Force-flush any remaining audio in the processor's buffer.
    ...
    if (this.audioContext && this.processorNode && this.sourceNode) {
      try {
        // Disconnect the real audio source
