...
const ToastViewport: React.FC<{
  ...
  return (
    <div
      ...
      onMouseEnter={() => {
        (window as any).electronAPI?.setMainWindowInteractivity?.(true);
      }}
      onMouseLeave={() => {
        (window as any).electronAPI?.setMainWindowInteractivity?.(false);
      }}
    >
      {toasts.map((toast) => (
  ...
  return (
    <div className={`${baseClasses} ${variantClasses[variant]}`}>
      ...
      {onClose && (
        <button
          onClick={onClose}
          className={`absolute right-2 top-2 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-border focus:ring-offset-2 ${iconClasses[variant]}`}
        >
          <X className="h-4 w-4" />
