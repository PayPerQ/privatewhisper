...
// as many native binaries (sherpa-onnx, ffmpeg) don't handle these paths correctly.
function getSafeTempDir() {
  ...
  const fallback = path.join(fallbackBase, "PPQ-Voice", "temp");
  ...
  try {
    ...
    const rootFallback = path.join(process.env.SystemDrive || "C:", "PPQ-Voice", "temp");
    try {
